...
const TYPE_FIELDS = {
  predication: [
    {key:'predicateur',label:'Predicateur',type:'text',placeholder:'Nom du predicateur'},
    {key:'texte_ref',label:'Texte biblique',type:'text',placeholder:'Ex : Jean 3:16'},
    {key:'serie',label:'Serie / Theme',type:'text',placeholder:'Nom de la serie'},
    {key:'duree',label:'Duree',type:'text',placeholder:'Ex : 45 min'},
  ],
  document: [
    {key:'nb_pages',label:'Nombre de pages',type:'text',placeholder:'Ex : 12 pages'},
    {key:'langue',label:'Langue',type:'text',placeholder:'Francais'},
    {key:'version',label:'Version',type:'text',placeholder:'v1.0'},
  ],
  formulaire: [
    {key:'usage',label:'Usage / Objectif',type:'text',placeholder:'Ex : Inscription nouveau membre'},
  ...
  media: [
    ...
    {key:'duree',label:'Duree',type:'text',placeholder:'Ex : 1h 20min'},
    {key:'resolution',label:'Qualite',type:'text',placeholder:'Ex : 1080p'},
  ],
}
...
export default function Ressources() {
  ...
  // ── REF CRITIQUE : stocke l'URL uploadée immédiatement ──────
  // setState est asynchrone → on ne peut pas lire form.url juste
  // après setForm(...). On stocke l'URL dans une ref synchrone
  // pour qu'elle soit disponible dès le clic sur "Enregistrer".
  const uploadedUrl    = useRef('')
  const uploadedTaille = useRef('')

  const load = async () => {
    setLoading(true)
    const [{data:r},{data:b}] = await Promise.all([
      ressourcesApi.getAll(),
      branchesApi.getAll()
    ])
    setAllItems(r||[])
    setBranches(b||[])
  ...
  const filtered = allItems.filter(i => {
    ...
    return (!search || (i.titre||'').toLowerCase().includes(q) || (i.auteur||'').toLowerCase().includes(q))
      && (!filterT || i.type === filterT)
  })

  const stats = {
    total:        allItems.length,
    predications: allItems.filter(i=>i.type==='predication').length,
    documents:    allItems.filter(i=>i.type==='document').length,
    formulaires:  allItems.filter(i=>i.type==='formulaire').length,
  }
  ...
  const openEdit = r => {
    ...
    setExtraMeta({})
    setUrlMode(!!r.url)
    // Pré-remplir la ref avec l'URL existante
    uploadedUrl.current    = r.url    || ''
    uploadedTaille.current = r.taille || ''
    setModal(true)
  }
  ...
  const handleFile = async e => {
    ...
    try {
      ...
      // 1. Stocker dans la ref IMMÉDIATEMENT (synchrone)
      uploadedUrl.current    = url
      uploadedTaille.current = size

      // 2. Mettre à jour l'UI (asynchrone, pas grave)
      setForm(p => ({ ...p, url, taille: size }))

    } catch(err) {
      console.error('Upload error:', err)
      alert('Erreur upload. Verifiez les politiques du bucket mers-files dans Supabase Storage.')
    }
    setUploading(false)
  }
  ...
  const save = async () => {
    ...
    // Lire l'URL depuis la ref (toujours à jour) OU depuis form.url
    // pour le cas "lien URL" manuel
    const finalUrl    = uploadedUrl.current || form.url || ''
    const finalTaille = uploadedTaille.current || form.taille || ''
    ...
    if (editing) {
      const { error } = await ressourcesApi.update(editing.id, payload)
      if (error) console.error('Update error:', error)
    } else {
      const { error } = await ressourcesApi.create(payload)
  ...
  return (
    <div>
      <PageHeader
        title="Ressources et Bibliotheque"
        subtitle={`${allItems.length} ressource(s)`}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Predications" value={stats.predications} accent="blue"   icon={c=><IMic  s={20} c={c}/>}/>
          ...
          {['','predication','document','formulaire','media'].map(t=>(
            <button key={t} onClick={()=>setFilterT(t)}
              className={`px-3 py-2 rounded-lg text-xs font-semibold border transition-all ${
                filterT===t ? 'bg-[#0D2B5E] text-white border-[#0D2B5E]' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'
              }`}>
      ...
      {loading ? <Spinner /> : filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.map(r => {
            ...
            return (
              <Card key={r.id}
                className={`transition-all ${hasFile?'hover:shadow-md cursor-pointer':'hover:shadow-sm'}`}
                onClick={()=>hasFile&&setPreview(r)}>
                <CardBody>
                  ...
                  {/* Indicateur fichier */}
                  {hasFile ? (
                    <div className="flex items-center gap-2 bg-[#EBF3FC] rounded-lg px-3 py-2 mb-3">
                      <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="#1A5EA8" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                      <span className="text-xs font-semibold text-[#1A5EA8]">Fichier joint — cliquer pour ouvrir</span>
                    </div>
                  ) : (
                    <div className="bg-slate-50 rounded-lg px-3 py-2 mb-3">
                      <span className="text-xs text-slate-300 italic">Aucun fichier joint</span>
                    </div>
                  )}
                      ...
                      {hasFile && (
                        <>
                          ...
                          <a href={r.url} target="_blank" rel="noopener noreferrer"
                            ...
                            <IconDownload size={11} color="#1A5EA8"/> Telecharger
                          </a>
                        </>
                      )}
                    </div>
                    <div className="flex gap-1">
                      <button onClick={e=>{e.stopPropagation();openEdit(r)}} className="p-1.5 rounded-lg text-slate-400 hover:bg-slate-100">
                        <IconEdit size={13}/>
                      </button>
                      <button onClick={e=>{e.stopPropagation();del(r)}} className="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-500">
                        <IconTrash size={13}/>
                      </button>
                    </div>
                  </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{background:typeBg[preview.type]}}>
                  {(()=>{ const I=typeIconMap[preview.type]||IFile; return <I s={16} c={typeIc[preview.type]}/> })()}
                </div>
                <div className="min-w-0">
                  <h2 className="text-sm font-bold text-[#0D2B5E] truncate">{preview.titre}</h2>
                  {preview.auteur && <p className="text-xs text-slate-400">Par {preview.auteur}</p>}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <a href={preview.url} target="_blank" rel="noopener noreferrer"
                  ...
                  <IconDownload size={12} color="#1A5EA8"/> Telecharger
                </a>
            ...
            <div className="flex-1 overflow-hidden rounded-b-2xl" style={{minHeight:'65vh'}}>
              {isPDF(preview.url) ? (
                <iframe src={preview.url} className="w-full h-full" style={{border:'none',minHeight:'65vh'}} title={preview.titre}/>
              ) : isImage(preview.url) ? (
                <div className="flex items-center justify-center h-full p-4 bg-slate-50">
                  <img src={preview.url} alt={preview.titre} className="max-w-full max-h-[70vh] object-contain rounded-lg shadow"/>
                </div>
              ) : (
                <iframe
                  src={`https://docs.google.com/viewer?url=${encodeURIComponent(preview.url)}&embedded=true`}
                  className="w-full" style={{border:'none',minHeight:'65vh'}} title={preview.titre}/>
              )}
            </div>
          </div>
        </div>
      ...
      <Modal isOpen={modal} onClose={close} title={editing?'Modifier la ressource':'Nouvelle ressource'} size="lg">
        <div className="space-y-4">
          ...
          <Select label="Type *" value={form.type}
            onChange={e=>{ setForm(p=>({...p,type:e.target.value})); setExtraMeta({}) }}>
            <option value="predication">Predication / Message</option>
            <option value="document">Document / Etude</option>
            <option value="formulaire">Formulaire</option>
            <option value="media">Media (Video / Audio)</option>
          </Select>

          <Input label="Titre *" value={form.titre} onChange={f('titre')} placeholder="Titre de la ressource"/>
          <Input label="Auteur / Source" value={form.auteur||''} onChange={f('auteur')} placeholder="Auteur..."/>

          {(TYPE_FIELDS[form.type]||[]).length > 0 && (
            <div className="bg-slate-50 rounded-xl p-4 space-y-3">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider">Details specifiques</p>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <div className="flex gap-1 bg-slate-100 rounded-lg p-0.5">
                <button onClick={()=>setUrlMode(false)}
                  className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${!urlMode?'bg-white text-[#0D2B5E] shadow-sm':'text-slate-500'}`}>
                  Upload
                </button>
            ...
            {!urlMode ? (
              <>
                ...
                <div
                  className="border-2 border-dashed border-slate-200 rounded-xl p-5 text-center hover:border-[#1A5EA8] transition-colors cursor-pointer"
                  onClick={()=>fileRef.current?.click()}>
                  {uploading ? (
                    <div className="flex items-center justify-center gap-2">
                      <div className="w-5 h-5 border-2 border-[#1A5EA8] border-t-transparent rounded-full animate-spin"/>
                      <span className="text-sm text-slate-500">Upload en cours...</span>
                    </div>
                  ) : uploadedUrl.current || form.url ? (
                    <div className="flex flex-col items-center gap-1">
                      <div className="w-10 h-10 bg-[#EAF3DE] rounded-xl flex items-center justify-center">
                        <svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="#3B6D11" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                      </div>
                      <span className="text-sm font-semibold text-[#3B6D11]">Fichier pret</span>
                      {(uploadedTaille.current||form.taille) && (
                        <span className="text-xs text-slate-400">{uploadedTaille.current||form.taille}</span>
                      )}
                      <span className="text-xs text-slate-400 mt-1">Cliquer pour changer</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-2">
                      <IUp s={28} c="#9AA5B4"/>
                      <p className="text-sm text-slate-400 font-medium">Cliquez pour choisir un fichier</p>
                      <p className="text-xs text-slate-300">PDF, Word, Excel, PowerPoint, Image, MP3, MP4</p>
                    </div>
                  )}
                </div>
              </>
              ...
              <Input value={form.url||''} onChange={e=>{
                f('url')(e)
                uploadedUrl.current = e.target.value
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input label="Taille" value={form.taille||''} onChange={f('taille')} placeholder="Ex : 2.4 Mo"/>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-50">
            ...
            <Button variant="gold" onClick={save} disabled={saving||!form.titre||uploading}>
              {uploading ? 'Upload...' : saving ? 'Enregistrement...' : editing ? 'Mettre a jour' : 'Ajouter'}
            </Button>
          </div>
        </div>
      </Modal>
